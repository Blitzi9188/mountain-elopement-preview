...
HELI_FAQ=[
 ("How much does a helicopter elopement cost?",
  "The flight starts at around &euro;2,500 and depends on the landing point, flight time and number of set-downs. Photography, planning and the rest of the day are quoted separately and bundled into one clear price.",
  "Was kostet ein Helikopter-Elopement?",
  "Der Flug beginnt bei etwa 2.500&nbsp;&euro; und hängt vom Landepunkt, der Flugzeit und der Zahl der Aufsetzer ab. Fotografie, Planung und der Rest des Tages werden separat kalkuliert und zu einem klaren Preis gebündelt."),
 ...
 ("Where can the helicopter land?",
  "Only on approved sites flown by licensed alpine operators &mdash; a ridge, a high meadow or a glacier plateau, not anywhere at will and not inside every protected zone. We confirm the exact spot before booking.",
  "Wo darf der Helikopter landen?",
  "Nur auf genehmigten Plätzen, geflogen von zugelassenen Alpin-Anbietern &mdash; einem Grat, einer Hochwiese oder einem Gletscherplateau, nicht überall und nicht in jeder Schutzzone. Den genauen Platz bestätigen wir vor der Buchung."),
 ("Is it legal to get married after a helicopter flight?",
  "The flight is the setting, not the legal act. Most couples hold a symbolic ceremony up top and sign the legal papers at home; a legal civil marriage in Italy or Austria is possible with paperwork started months ahead.",
  "Ist die Trauung nach einem Helikopterflug rechtsgültig?",
  "Der Flug ist der Rahmen, nicht der Rechtsakt. Die meisten Paare feiern oben eine symbolische Zeremonie und unterschreiben die Papiere zu Hause; eine standesamtliche Trauung in Italien oder Österreich ist mit rechtzeitig begonnenen Papieren möglich."),
 ...
 ("How many people can come along?",
  "Usually just the two of you, sometimes a witness or two &mdash; seats are limited on a small helicopter and every passenger affects the flight. We plan the headcount with the operator.",
  "Wie viele Personen können mitfliegen?",
  "Meist nur ihr beide, manchmal ein bis zwei Trauzeugen &mdash; die Plätze in einem kleinen Helikopter sind begrenzt und jede Person wirkt sich auf den Flug aus. Die Personenzahl planen wir mit dem Anbieter."),
]
...
def build_helicopter_guide(lang):
    ...
    C.append('<!-- FAKT PRÜFEN: max. Passagierzahl/Trauzeugen pro Flug beim genutzten Anbieter -->')
    ...
    C.append('<!-- FAKT PRÜFEN: exakte Sonnenaufgangs-/Golden-Hour-Zeiten je Monat pro Datum bestätigen -->')
    ...
    C.append(p("Plan from around <strong>&euro;2,500 for the flight</strong> itself, depending on the landing point, the flight time and the number of set-downs. A remote or glacier landing costs more than a nearby ridge; a second location or a longer window adds to it. That is the flight only &mdash; photography, planning and the rest of the day are separate, and we bundle everything into one clear quote. Because weather can move the date, we always hold a flexible window and a beautiful ground-based Plan B, so a grounded morning never means a lost day.","Rechnet ab etwa <strong>2.500&nbsp;&euro; für den Flug</strong> selbst &mdash; je nach Landepunkt, Flugzeit und Zahl der Aufsetzer. Eine abgelegene oder Gletscherlandung kostet mehr als ein naher Grat; eine zweite Location oder ein längeres Fenster kommt dazu. Das ist nur der Flug &mdash; Fotografie, Planung und der Rest des Tages sind separat, und wir bündeln alles in ein klares Angebot. Weil das Wetter den Termin verschieben kann, halten wir immer ein flexibles Fenster und einen schönen Plan B am Boden frei, damit ein geerdeter Morgen nie ein verlorener Tag ist."))
    C.append('<!-- FAKT PRÜFEN: aktuelle Flugpreise + was genau enthalten ist; Aufpreis Gletscherlandung -->')
    ...
    C.append(p("It is worth being honest about what drives the number. The bigger levers are the landing point (near vs. remote, meadow vs. glacier), the total flight time, and whether the pilot waits on the mountain or flies a second rotation to collect you. Add-ons &mdash; a second location, film, flowers, a celebrant &mdash; sit on top, and we price them plainly so nothing is a surprise on the invoice. A helicopter day is not the cheapest way to elope, but it buys something specific: an hour on a summit that would otherwise cost you two days and a lot of luck.","Man sollte ehrlich sagen, was den Preis treibt. Die größeren Hebel sind der Landepunkt (nah vs. abgelegen, Wiese vs. Gletscher), die gesamte Flugzeit und ob der Pilot am Berg wartet oder eine zweite Runde fliegt, um euch abzuholen. Zusatzoptionen &mdash; eine zweite Location, Film, Blumen, eine freie Trauung &mdash; kommen obendrauf, und wir kalkulieren sie transparent, damit auf der Rechnung nichts überrascht. Ein Helikoptertag ist nicht der günstigste Weg zu heiraten, aber er kauft etwas Bestimmtes: eine Stunde auf einem Gipfel, der euch sonst zwei Tage und viel Glück kosten würde."))
    ...
    C.append('<!-- FAKT PRÜFEN: konkret genehmigte Landeplätze (Gletscher, Nationalpark-Grenzen); erlaubte Zonen bei Brixen/Tre Cime -->')
    ...
    C.append('<!-- FAKT PRÜFEN: konkreter Start-Helipad (Brixen/Dolomiten) des Anbieters; Fahrzeit Helipad->Landezonen -->')
    ...
    C.append('<!-- FAKT PRÜFEN: genaue Dokumentenlisten und Vorlaufzeiten je Land (DE/AT/US/UK); ob die rechtsgültige Trauung am Landeplatz möglich ist oder nur am Standesamt -->')
